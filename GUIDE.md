...
- [ ] Checked `_data/sources.json` — add or update the entry for this source, being honest about the licence, with a short `licence_type` (such as "OGL v3.0", "ODbL" or "None stated") and its `data` files, each with its `schedule` (`every 5 minutes`, `hourly`, `every two hours`, `daily`, `monthly` or `manual`). The local admin page's Data Freshness tab reads these to flag data that has stopped updating, and `_python/tests/test_sources_registry.py` fails if a data file isn't listed
